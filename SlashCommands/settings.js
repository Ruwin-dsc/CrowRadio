...
module.exports = {
    ...
    run: async (bot, interaction, args, config, ephemeral) => {
        ...
        if(req.mode == 'automatic') embed.addFields({ name: 'Salon', value: interaction.guild.channels.cache.get(req.channelAutomatic) ? interaction.guild.channels.cache.get(req.channelAutomatic) : 'Aucun', inline: true })
